...
public class MultiThreadTr {
     ...
     * @return 返回所有事务的trace
     */
    public static String RunAndGetTrace(int sessionNum, int trNum, int opNum) throws InterruptedException {
        //实例化一个对象进行数据库的初始化
        Session test0 = new Session(1,0, "S0");
        test0.initial();
        test0.interrupt();
        //使用多线程注入事务，trNum为事务的数量，opNum为每个事务进行的随机操作数量
        String trace = null;
        for(int i = 1; i <= sessionNum; i++){
            new Session(trNum, opNum, "S" + i).start();
        }
        while(true) {
