...
public class Session extends Thread{
    /**
     * 生成事务并注入MySQL数据库执行的线程
     *
     */
    private Connection connection;
    private int trNum;
    private int num;//每个事务里的操作数
    private String name;
    ...
    //mysql数据库连接并初始化参数，事务的操作数，名称与隔离级别
    public Session(int trnum, int num, String name, String IsolationLevel){
        this.trNum = trnum;
        this.num = num;
        this.name = name;
        this.connection = MySQLDBMS.getConnection();
...
//            System.out.println("关闭数据库资源成功！");
    ...
    private void SetIsolationLevel(String level){
        String sql = "set session transaction isolation level " + level;
        Statement stmt = null;
        try {
            stmt = connection.createStatement();
            stmt.execute(sql);
